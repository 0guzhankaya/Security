...
namespace Security.Service.Services
{
	public class AuthenticationService : IAuthenticationService
	{
		...
		public async Task<Response<TokenDto>> CreateTokenAsync(LoginDto loginDto)
		{
			...
			if (userRefreshToken == null)
			{
				await _userRefreshTokenService.AddAsync(
					new UserRefreshToken { UserId = user.Id, Code = token.RefreshToken, Expression = token.RefreshTokenExpiration });
			}
			else
			{
				userRefreshToken.Code = token.RefreshToken;
				userRefreshToken.Expression = token.RefreshTokenExpiration;
			}
		...
		public async Task<Response<TokenDto>> CreateTokenByRefreshToken(string refreshToken)
		{
			...
			existRefreshToken.Expression = tokenDto.RefreshTokenExpiration;
